...
def post_data(param):
    ...
    if 'url_params' in param['api_params']:
        url_params = api_params['url_params']
    try:            
        response = requests.post(full_url, auth=(username,password), headers=headers, params=url_params, data=body_data, verify=False)
        if response.status_code == 200:
            json_response = response.json()
            return json_response['result']
        else:
...
def put_data(param):
    ...
    if 'url_params' in param['api_params']:
        url_params = api_params['url_params']
    try:            
        response = requests.put(full_url, auth=(username,password), headers=headers, params=url_params, data=body_data, verify=False)
        if response.status_code == 200:
            json_response = response.json()
            return json_response['result']
        else:
